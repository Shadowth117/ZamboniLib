...
namespace Zamboni.IceFileFormats
{
    public class IceV5File : IceFile
    {
      ...
      V9Decrypt
        ...
        public IceV5File(string filename) => loadFile(File.OpenRead(filename));

        public IceV5File(Stream inStream) => loadFile(inStream);
        ...
        private int calculateKeyStep1()
        {
            ...
            uint temp_key = calcBlowfishKeys(magicNumbers, getKey(magicNumbers, (uint)((int)BitConverter.ToUInt32(((IEnumerable<byte>)new Crc32().ComputeHash(magicNumbers, crcStartPos, count)).Reverse().ToArray(), 0) ^ (int)BitConverter.ToUInt32(magicNumbers, keyStartPos) ^ fileSize ^ 1129510338)));
            uint key = getKey(magicNumbers, temp_key);
            BitConverter.ToUInt32(((IEnumerable<byte>)BitConverter.GetBytes(temp_key << decryptionHeaders[iceType - 5].HeaderRol | temp_key >> 32 - decryptionHeaders[iceType - 5].HeaderRol)).Reverse().ToArray(), 0);
            uint num1 = temp_key >> decryptionHeaders[iceType - 5].HeaderRol | temp_key << 32 - decryptionHeaders[iceType - 5].HeaderRol;
            uint num2 = key >> decryptionHeaders[iceType - 5].HeaderRol | key << 32 - decryptionHeaders[iceType - 5].HeaderRol;
            return 0;
        }
        ...
        private uint getKey(byte[] keys, uint temp_key)
        {
            ...
            return (uint)((byte)((keys[(int)num3] << num8 | keys[(int)num3] >> 8 - num8) & byte.MaxValue) << 24 | (byte)((keys[(int)num1] << num7 | keys[(int)num1] >> 8 - num7) & byte.MaxValue) << 16 | (byte)((keys[(int)num2] << num5 | keys[(int)num2] >> 8 - num5) & byte.MaxValue) << 8) | (byte)((keys[(int)num4] << num6 | keys[(int)num4] >> 8 - num6) & byte.MaxValue);
        }
